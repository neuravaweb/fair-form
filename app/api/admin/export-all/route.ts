...
export async function GET(request: NextRequest) {
  try {
    ...
    submissions.forEach((submission, index) => {
      ...
      doc.text(`Zgloszenie ${index + 1} z ${submissions.length}`, 20, yPos)
      yPos += 10
      ...
      if (submission.notes) {
        doc.setTextColor(0, 0, 0) // Black
        const notesLines = doc.splitTextToSize(`Notatka: ${submission.notes}`, 170)
    ...
    return new NextResponse(pdfBuffer, {
      headers: {
        ...
        'Content-Disposition': `attachment; filename="all-submissions-${new Date().toISOString().split('T')[0]}.pdf"`,
      },
    })
  } catch (error) {
